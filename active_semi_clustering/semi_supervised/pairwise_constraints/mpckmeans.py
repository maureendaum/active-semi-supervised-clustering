...
class MPCKMeans:
    ...
    def __init__(self, n_clusters=3, max_iter=10, w=1, rng=None):
    ...
    def fit(self, X, y=None, ml=set(), cl=set(), random_init=False, stop_early=True):
        ...
        for iteration in range(self.max_iter):
            ...
            # Assign clusters
            labels = self._assign_clusters(X, y, cluster_centers, A, farthest, ml_graph, cl_graph, self.w, self.labels[-1] if len(self.labels) else np.full(X.shape[0], fill_value=-1))
            ...
            for x_i in range(X.shape[0]):
                i_error, i_d, i_m, i_c = self._objective_fn(X, x_i, labels, cluster_centers, labels[x_i], A, farthest, ml_graph, cl_graph, self.w)
                error += i_error
                term_d += i_d
                term_m += i_m
                term_c += i_c
    ...
    def _objective_fn(self, X, i, labels, cluster_centers, cluster_id, A, farthest, ml_graph, cl_graph, w):
        term_d = self._dist(X[i], cluster_centers[cluster_id], A) - np.log(np.linalg.det(A))  # FIXME is it okay that it might be negative?
    ...
    def _assign_clusters(self, X, y, cluster_centers, A, farthest, ml_graph, cl_graph, w, labels, should_print=False):
        previous_labels = labels.copy()
        index = list(range(X.shape[0]))
        self.rng.shuffle(index)
        for i in index:
            distances = [self._objective_fn(X, i, labels, cluster_centers, cluster_id, A, farthest, ml_graph, cl_graph, w)[0] for cluster_id, cluster_center in enumerate(cluster_centers)]
